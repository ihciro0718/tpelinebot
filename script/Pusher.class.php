...
abstract class Pusher
{
    ...
    {
        // fetch pushable member list if last changed in 1 hour and not been pushed yet
        // 2016-08-14 get by dataset_to_push changed_at
        $query = "SELECT * FROM `subscription_container`
                  WHERE `is_pushed` = 0
                  AND `dataset_id` = '" . $this->datasetId . "'";
        // AND (" . $this->currentTimestamp . " - IF(UNIX_TIMESTAMP(`last_pushed_at`) IS NULL, UNIX_TIMESTAMP(NOW()), UNIX_TIMESTAMP(`last_pushed_at`))) /60 /60 > 1;";
